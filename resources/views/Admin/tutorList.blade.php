...
@section('content')
    ...
    <form method="GET" action="{{ route('tutorList') }}" class="mb-4">
        <div class="flex items-center">
            <input type="text" name="search" value="{{ request()->input('search') }}" placeholder="Cari berdasarkan id, nama, atau email" class="border px-4 py-2 rounded-lg mr-2 w-96">
                ...
                @foreach ($users as $user)
                    <tr>
                        ...
                        <td class="border border-gray-300 px-4 py-2">
                            {{ optional($user)->subject_name ?? 'Tidak ada mata pelajaran' }}
                        </td>
                        <td class="border border-gray-300 px-4 py-2">
